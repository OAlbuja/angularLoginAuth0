...
export class ClienteInteresFormComponent implements OnInit {
  ...
  create(): void {
    this.clienteInteresService.create(this.clienteInteres)
      .subscribe(clienteInteres => {
        ...
        swal.fire('Nuevo Cliente-Interes', `ClienteInteres ${clienteInteres.clienteApellido} creado con éxito`, 'success');
      });
  }
  ...
  update(): void {
    this.clienteInteresService.update(this.clienteInteres)
      .subscribe(clienteInteres => {
        this.router.navigate(['/clientes-interes'])
        swal.fire('Cliente Actualizado', `ClienteInteres ${clienteInteres.clienteApellido} actualizado con éxito!`, 'success');
      }
